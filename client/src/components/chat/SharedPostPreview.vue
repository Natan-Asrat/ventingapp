<template>
  <div 
    ...
    :class="isMine ? 'bg-zinc-800 border border-zinc-700' : 'bg-zinc-50 border border-zinc-200'"
  ...
  >
    ...
    <div class="p-3 pb-2 flex items-center space-x-2">
        ...
        <div v-else class="h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0" :class="isMine ? 'bg-zinc-700 text-zinc-300' : 'bg-zinc-200 text-zinc-500'">
    ...
    <!-- Footer -->
    <div class="px-3 py-2 flex items-center justify-between text-[10px]" :class="isMine ? 'bg-zinc-900/50 text-zinc-400' : 'bg-zinc-100 text-zinc-500'">
        <span>Post</span>
...
const navigateToPost = (e) => {
  // Don't navigate if clicking on "Show more" button inside ShowMore component
  if (e.target.closest('button') || e.target.closest('.cursor-pointer')) return;
  
  if (props.post?.id) {
    router.push({ name: 'Feed', query: { p: props.post.id } });
  }
};
</script>